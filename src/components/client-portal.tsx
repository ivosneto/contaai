...
import { PortalStoreProvider, usePortalStore } from "@/data/portal-store";
import { getPortalDocumentDownloadUrlFn } from "@/data/server-functions/portal";
...
// accountsReceivable/accountsPaid/meetings ainda vêm de office.ts (Financeiro
// não foi migrado nesta tarefa — mesmo escopo já documentado na tarefa
// anterior). Isso não vaza entre tenants: os ids de cliente de um workspace
// de teste novo nunca coincidem com os ids estáticos ("c1".."c20") deste
// arquivo, então a aba correspondente aparece vazia, não com dado de outro
// tenant — mas é uma limitação a resolver quando o Financeiro for religado.
...
/** clientId vem exclusivamente de usePortalStore() (sessão autenticada, papel 'client') — nunca de uma constante fixa ou de qualquer coisa vinda do navegador. */
      ...
      <header className="sticky top-0 z-30 border-b border-glass-line/60 bg-background/85 px-4 py-3 backdrop-blur-md sm:px-6">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              className="grid size-9 shrink-0 place-items-center rounded-full bg-linear-to-br from-brand/20 to-accent/20 text-sm font-semibold text-brand"
              title="Sair"
...
type PortalUploadForm = Omit<NewDocumentInput, "file"> & { file: File | null };
...
function ClientPortalContent({ clientId }: { clientId: string }) {
  ...
  const [uploadForm, setUploadForm] = useState<PortalUploadForm>({
    clientId,
    type: "Nota fiscal",
  ...
  const doneItems = pendencies.filter(
    (p) => p.clientId === clientId && (p.status === "Concluída" || p.status === "Cancelada"),
  );
  ...
  const relevantAnnouncements = announcements.filter(
    (a) => a.audience === "Todos os clientes" || a.audience === client.department,
  );
  const upcomingObligations30d = cObligations.filter(
    (o) => o.status !== "Concluída" && o.dueDate >= "2026-09-14" && o.dueDate <= "2026-10-14",
  ).length;

  const runDownload = async (documentId: string) => {
    setDownloadingIds((s) => new Set(s).add(documentId));
    try {
      const { url } = await getPortalDocumentDownloadUrlFn({ data: { documentId } });
  ...
  const submitUpload = async () => {
    ...
    try {
      await uploadDocument({
        ...
        ...(uploadContext ? { pendencyId: uploadContext.pendencyId } : {}),
      });
      setUploadOpen(false);
    } finally {
      setUploading(false);
  ...
  const heroItems: HeroItem[] = [
    ...pendingItems.map((p) => ({
      ...
      dueDate: p.dueDate,
      action: p.category === "Documento" ? ("documento" as const) : ("resposta" as const),
      pendency: p,
    })),
    ...waitingObligations.map((o) => ({
      id: `ob-${o.id}`,
      title: `${o.type} — competência ${o.competence}`,
      detail: `Aguardando um documento ou confirmação sua para avançar. Vence em ${o.dueDate}.`,
      dueDate: o.dueDate,
      action: "documento" as const,
    })),
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Badge tone="good">
            <ShieldCheck className="mr-1 size-3" /> Acesso só aos seus dados
          </Badge>
        }
      />
      ...
      <Glass className="p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Documentos e informações pendentes para não atrasar nenhum prazo.
        </p>
        {heroItems.length === 0 ? (
              ...
              <div key={item.id} className="glass-soft rounded-xl p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  {item.action === "documento" ? (
                    <Button size="sm" onClick={() => openUpload(item.pendency?.id)}>
                      <Upload /> Enviar documento
                    </Button>
                  ) : (
                    <Button
                      size="sm"
                      variant={replyTarget === item.id ? "secondary" : "default"}
                ...
                {replyTarget === item.id && item.pendency && (
                  <div className="mt-3 space-y-2">
                    ...
                    <Button
                      ...
                      onClick={() => submitReply(item.pendency!.id, item.title)}
                    >
                      Enviar resposta
                    </Button>
                  </div>
        ...
        <TabsContent value="inicio">
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            <Kpi
              ...
              value={brl(client.fee)}
              change={client.services.join(", ")}
              tone="brand"
            ...
            <Kpi
              ...
              value={paymentStatus}
              change={
                cReceivable.length > 0 ? `${cReceivable.length} fatura(s) em aberto` : "tudo em dia"
              }
              tone={
                paymentStatus === "Vencido" ? "bad" : paymentStatus === "Pendente" ? "warn" : "good"
              }
              icon={CircleDollarSign}
            />
            <Kpi
              label="Documentos pendentes"
              value={String(heroItems.filter((i) => i.action === "documento").length)}
              change="aguardando envio"
              tone={heroItems.some((i) => i.action === "documento") ? "warn" : "good"}
              icon={FileText}
            />
            <Kpi
              label="Obrigações em 30 dias"
              value={String(upcomingObligations30d)}
              change="a vencer"
              tone={upcomingObligations30d > 0 ? "warn" : "good"}
              icon={Gauge}
            />
          </div>
          <Glass className="mt-4 p-5">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              ...
              <div className="glass-soft rounded-xl p-4">
                <p className="text-[10px] uppercase text-muted-foreground">Cliente desde</p>
                <p className="mt-1 font-semibold">{client.since.slice(0, 4)}</p>
              </div>
              <div className="glass-soft rounded-xl p-4">
                <p className="text-[10px] uppercase text-muted-foreground">Serviços contratados</p>
                <p className="mt-1 font-semibold">{client.services.length}</p>
              </div>
            </div>
          </Glass>
        </TabsContent>
            ...
            {cDocuments.length === 0 ? (
              ...
              <div className="mt-3 space-y-2">
                {cDocuments.map((d) => (
                  <div
                    key={d.id}
                    className="glass-soft flex flex-wrap items-center justify-between gap-2 rounded-xl p-3"
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-semibold">{d.name}</p>
                      <p className="text-xs text-muted-foreground">
                        Competência {d.competence} · enviado em {d.uploadedAt} · {d.pipelineStage}
                      </p>
                    </div>
                    ...
                    >
                      <div className="min-w-0">
                        <p className="text-sm font-semibold">
                          {o.type} · competência {o.competence}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Vence em {o.dueDate} · responsável {o.assignee} · checklist{" "}
            ...
            <Glass className="mt-4 p-5">
              ...
              <div className="mt-3 space-y-2">
                {doneItems.slice(0, 10).map((p) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between gap-2 rounded-xl bg-glass p-3"
                  >
        ...
        <TabsContent value="solicitacoes">
          <Glass className="p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => setTab("mensagens")}>
                <MessageSquare /> Nova solicitação
              </Button>
            </div>
            {requests.length === 0 ? (
              <p className="mt-3 text-sm text-muted-foreground">
                Você ainda não abriu nenhuma solicitação. Use "Mensagens" para falar com a gente.
              </p>
            ) : (
                ...
                <div
                  ...
                  className={
                    m.direction === "Recebida"
                      ? "ml-auto max-w-[85%] rounded-xl rounded-tr-sm bg-brand/10 p-3"
                      : "mr-auto max-w-[85%] rounded-xl rounded-tl-sm bg-glass p-3"
                  }
                >
                  <p className="text-sm">{m.content}</p>
                  <p className="mt-1 text-[10px] text-muted-foreground">
                    {m.direction === "Recebida" ? "Você" : m.sender} · {m.createdAt}
                  </p>
                </div>
              ))}
            </div>
        ...
        <TabsContent value="financeiro">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <Kpi
              ...
              value={String(cReceivable.length)}
              change="aguardando pagamento"
              tone={cReceivable.length > 0 ? "warn" : "good"}
              icon={ClipboardList}
            />
          </div>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <Glass className="p-5">
              ...
              {cReceivable.length === 0 ? (
                <p className="mt-2 text-sm text-muted-foreground">Nenhuma fatura em aberto.</p>
              ) : (
                <div className="mt-3 space-y-2">
                  {cReceivable.map((r) => (
                    <div
                      key={r.invoiceId}
                      className="glass-soft flex items-center justify-between rounded-xl p-3"
                    >
                      <div>
                        <p className="text-sm font-semibold">{brl(r.amount)}</p>
                        <p className="text-xs text-muted-foreground">Vence {r.dueDate}</p>
                      </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setUploadForm((f) => ({ ...f, competence: e.target.value }))}
              />
            </div>
            <div>
              <label className="text-xs font-semibold uppercase text-muted-foreground">
            ...
            <p className="text-xs text-muted-foreground">
              O arquivo é enviado para o armazenamento do escritório — só quem tem permissão sobre
              este cliente consegue acessá-lo depois.
            </p>
          </div>
          <DialogFooter>
            ...
            <Button onClick={() => void submitUpload()} disabled={!uploadForm.file || uploading}>
