...
async function fromRow(workspaceId: string, row: ProcessRow, steps: ProcessStep[]): Promise<Process> {
  return {
    id: row.id,
    name: row.name,
...
export async function listProcesses(workspaceId: string): Promise<Process[]> {
  const { data: rows, error } = await supabaseDomain.from("processes").select("*").eq("workspace_id", workspaceId).order("name");
  if (error) throw new Error(`Falha ao listar processos: ${error.message}`);
  const ids = (rows ?? []).map((r) => r.id);
  const { data: stepRows, error: stepError } = ids.length
    ? await supabaseDomain.from("process_steps").select("*").in("process_id", ids)
  ...
  return Promise.all((rows ?? []).map((row) => fromRow(workspaceId, row, stepsFromRows(byProcess.get(row.id) ?? []))));
}

export async function upsertProcess(workspaceId: string, process: Process): Promise<void> {
  const departmentId = await departmentIdFor(workspaceId, process.department);
  const { error } = await supabaseDomain.from("processes").upsert({
    id: process.id,
  ...
  const { error: deleteError } = await supabaseDomain.from("process_steps").delete().eq("process_id", process.id);
  if (deleteError) throw new Error(`Falha ao atualizar etapas do processo ${process.id}: ${deleteError.message}`);
  if (process.steps.length > 0) {
    const { error: insertError } = await supabaseDomain.from("process_steps").insert(
