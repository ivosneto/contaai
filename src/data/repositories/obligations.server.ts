...
async function fromRow(
  ...
  return {
    ...
    department: (await departmentNameFor(client, workspaceId, row.department_id)) ?? "Fiscal",
...
function checklistFromRows(rows: ObligationChecklistItemRow[]): ChecklistItem[] {
  return [...rows]
    ...
    .map((r) => ({ id: r.id, label: r.label, done: r.done }));
}
...
export async function listObligations(
  ...
  const { data: obligationRows, error } = await client
    .from("obligations")
    .select("*")
  ...
  const { data: checklistRows, error: checklistError } = ids.length
    ? await client.from("obligation_checklist_items").select("*").in("obligation_id", ids)
    : { data: [] as ObligationChecklistItemRow[], error: null };
  if (checklistError)
    throw new Error(`Falha ao listar checklist de obrigações: ${checklistError.message}`);
  ...
  return Promise.all(
    rows.map((row) =>
      fromRow(client, workspaceId, row, checklistFromRows(byObligation.get(row.id) ?? [])),
    ),
  );
}
...
export async function upsertObligation(
  ...
  const departmentId = await departmentIdFor(client, workspaceId, obligation.department);
  const { error } = await client.from("obligations").upsert({
    id: obligation.id,
    ...
    // Gravado de verdade desde a fatia de OCR/Document Intelligence — o
    // único escritor real é executeApprovedAiAction (kind
    // "link-obligation-evidence", src/data/server-functions/copilot.ts),
    // sempre depois de aprovação humana explícita da sugestão da IA.
    evidence_document_id: obligation.evidenceDocumentId,
  });
  if (error) throw new Error(`Falha ao salvar obrigação ${obligation.id}: ${error.message}`);

  if (obligation.checklist.length > 0) {
    const { error: checklistError } = await client.from("obligation_checklist_items").upsert(
    ...
    if (checklistError)
      throw new Error(
        `Falha ao salvar checklist da obrigação ${obligation.id}: ${checklistError.message}`,
      );
  }
}
